...
def compute() -> pd.DataFrame:
    df = read_csv_file(raw_path("futures_member_long_short_daily.csv"))
    df = df[(df["date"] <= pd.Timestamp("2025-04-30")) & (df["underlying"] == "IF")].copy()
    df["raw_value"] = df["long_short_ratio"]
    ...
    save_csv_file(out, processed_signal_path("futures_member"))
    return out
